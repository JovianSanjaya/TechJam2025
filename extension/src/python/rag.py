...
def process_compliance_analysis(legal_docs_path: str, features: List[Dict], use_cache: bool = True) -> Dict:
    ...
    try:
        ...
        print(f"✅ Vector store initialized ({'ChromaDB' if VECTOR_STORE_AVAILABLE else 'Fallback'})")
    except Exception as e:
...
def test_compliance_flags(legal_data):
    feature_samples = [
        {
    ...
    legal_statutes = extract_legal_statutes(legal_data)
    print(f"Found {len(legal_statutes)} legal documents to analyze")
    
    all_results = []
    
    for feature in feature_samples:
        print(f"Processing Feature: {feature['feature_name']}")
        compliance_flags = flag_feature_for_compliance(feature['feature_description'], legal_statutes[:3])  # Limit for legacy mode
        ...
        for flag in compliance_flags:
            print(f"Statute: {flag['statute']}")
            print(f"Compliance Flag: {flag['compliance_flag']}")
            print("="*50)
...
# 7. Enhanced Main Execution
    ...
    for feature in feature_samples:
        print(f"Processing Feature: {feature['feature_name']}")
        compliance_flags = flag_feature_for_compliance(feature['feature_description'], legal_statutes)
...
# 7. Enhanced Main Execution with Vector Search and Structured Output
def main():
    ...
    try:
        ...
        fallback_analysis(legal_documents_path, feature_samples)
...
def fallback_analysis(legal_docs_path: str, features: List[Dict]):
    """Fallback to original analysis method"""
    legal_data = load_legal_documents(legal_docs_path)
    legal_statutes = extract_legal_statutes(legal_data)
    
    with open('output.txt', 'w', encoding='utf-8') as f:
        f.write("=== FALLBACK COMPLIANCE ANALYSIS ===\n\n")
        
        for feature in features:
            f.write(f"FEATURE: {feature['feature_name']}\n")
            f.write(f"DESCRIPTION: {feature['feature_description']}\n\n")
            
            compliance_flags = flag_feature_for_compliance(
                feature['feature_description'],
                legal_statutes[:3]  # Limit to first 3 for fallback
            )
            
            for flag in compliance_flags:
                f.write(f"STATUTE: {flag['statute']}\n")
                f.write(f"ANALYSIS: {flag['compliance_flag']}\n")
                f.write("-" * 50 + "\n")
